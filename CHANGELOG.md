...
- **n8n** - Fix `Missing process handler for job type job` (defense-in-depth against any Bull-based neighbour sharing Redis) and `Task request timed out after 60 seconds` (long Code-node executions) by setting `QUEUE_BULL_PREFIX=n8n` to namespace n8n's queue keys, raising `N8N_RUNNERS_TASK_REQUEST_TIMEOUT` to 300s, and disabling task runner auto-shutdown (`N8N_RUNNERS_AUTO_SHUTDOWN_TIMEOUT=0`). Default `N8N_RUNNERS_MAX_CONCURRENCY` raised 5 → 10. All four values are configurable via `.env`.
